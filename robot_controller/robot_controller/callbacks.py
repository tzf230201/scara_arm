...
Callback handlers for commands coming from the tester UI.

The handlers expect a dict-like message with keys:
- command, motor, time, joints, coor, csv_path

In ROS 2 mode, robot_controller/main.py converts scara_arm_msgs/TesterUICommand into
this dict shape and routes through HANDLERS.
"""
...
def out2_active(msg, state):
    print(f"out2 active (not working yet)")
...
def out2_nonactive(msg, state):
    print(f"out2 non-active (not working yet)")
    # # servo_get_output_pin_configuration(2)
    # servo_get_digital_output_state()
    # servo_get_digital_output_state()
    # # servo_set_digital_output_state(2, 0)
