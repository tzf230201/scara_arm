...
{
    // Initialize ROS node
    ros::init(argc, argv, "zlis42_receiver");
    ros::NodeHandle nh;
	ros::Publisher chatter_pub = nh.advertise<std_msgs::String>("zlis42_receiver/chatter", 1000);
    // Create publisher for motor1 current position on "motor1_position"
    ros::Publisher motor1_position_pub = nh.advertise<std_msgs::Float64>("motor2/position", 1000);

    // Create publisher for motor1 speed on "motor1_speed"
    ros::Publisher motor1_speed_pub = nh.advertise<std_msgs::Float64>("motor2/speed", 1000);

    // Create publisher for motor1 current on "motor1_current"
    ros::Publisher motor1_current_pub = nh.advertise<std_msgs::Float64>("motor2current", 1000);

    // Create an instance of CanBusReader
    CanReader canReader("can0"); // Replace with your CAN interface name
    ...
    while (ros::ok())
    {
        ...
        if (canReader.read(frame)) {
            if ((frame.can_id != 0x602) && (frame.can_id != 0x603) && (frame.can_id != 0x604))
            { 
                //std_msgs::String msg;
                //std::stringstream ss;

                //ss << "CAN Frame ID: " << std::hex << frame.can_id << std::dec << " Data: ";
                //for (int i = 0; i < frame.can_dlc; ++i) {
                //    ss << std::hex << static_cast<int>(frame.data[i]) << " ";
                //}
                //msg.data = ss.str();

                // Publish the filtered CAN frame data
                //ROS_INFO("%s", msg.data.c_str());
                //chatter_pub.publish(msg); // Uncomment if you want to publish the filtered CAN data
