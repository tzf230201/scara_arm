...
def send_can1(request_id, function_code, internal_addr, data):
    send_can_command(f"{request_id:03X}#00{function_code:02X}00{internal_addr:02X}{data:08X}")

def send_can0(request_id, set_length, index, data):
...
#ret = send_can0(ID3, READ_4_BYTE, INDEX_POSITION_ACTUAL_VALUE, 0)
#print(f"{ret}")

def read_servo_angle(request_id):
	pulse = send_can0(request_id, READ_4_BYTE, INDEX_POSITION_ACTUAL_VALUE, 0)
	joint = pulse_to_degree(pulse)
	if (request_id != ID4):
		joint = joint * -1.0
	return joint
...
def read_servo_speed(request_id):
	vel = send_can0(request_id, READ_4_BYTE, INDEX_VELOCITY_ACTUAL_VALUE, 0)
	if (request_id != ID4):
		vel = vel * -1.0
	return vel
#qt
def is_target_reached(request_id):
	ret = send_can0(request_id, READ_2_BYTE, INDEX_STATUS_WORD, 0)
    ...
    for request_id in [ID2, ID3, ID4]:
        ...
        if response_id in cur_pulse_dict:
            ...
            if len(data_bytes) >= 4:
                ...
                angles.append(pulse_to_degree(data_int))
            else:
                angles.append(0)
        else:
            angles.append(0)
...
def init_motor(accel_time, decel_time, speed2, speed3, speed4):
	acc_dec_time = merge_acc_dec(accel_time, decel_time)
        max_speed_motor1 = speed_to_hex(max_speed)
        ...
        send_can1(ID1, FUNCTION_WRITE, ADDR_TRAPEZOID_SPEED, max_speed_motor1)
	...
	for id in [ID2, ID3, ID4]:
		send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x00) #init 0
		send_can0(id, SET_4_BYTE, INDEX_PROFILE_ACCELERATION, accel_time) #profile accel
		send_can0(id, SET_4_BYTE, INDEX_PROFILE_DECELERATION, decel_time) #profile decel
		send_can0(id, SET_1_BYTE, INDEX_MODE_OF_OPERATION, 0x01) #position mode
	
	for id, speed in zip([ID2, ID3, ID4], [speed2, speed3, speed4]):
		send_can0(id, SET_4_BYTE, INDEX_PROFILE_VELOCITY, speed) #profile speed

	for id in [ID2, ID3, ID4]:
		send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x06) #init 1
		send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x07) #init 2
		#send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x4F) #init 2
...
def relative_command(cur_joint2, cur_joint3, cur_joint4, tar_joint2, tar_joint3, tar_joint4):
	try:
	    	...
	    	pulse2 = degree_to_pulse(degree2)
	    	pulse3 = degree_to_pulse(degree3)
	    	pulse4 = degree_to_pulse(degree4)
	    	...
	    	for id in [ID2, ID3, ID4]:
	    		...
	    		send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, start_command)
	    	...
	    	for id, pulse in zip([ID2, ID3, ID4], [pulse2, pulse3, pulse4]):
	    		...
	    		send_can0(id, SET_4_BYTE, INDEX_TARGET_POSITION, pulse)
	    	
	    	for id in [ID2, ID3, ID4]:
	    		#send_can_command(f"{id:03X}#2B4060005F000000")
	    		send_can0(id, SET_2_BYTE, INDEX_CONTROL_WORD, execute_command)
...
def send_relative_command():
	try:
	    	tar_joint1= float(entry_motor1.get())
	    	tar_joint2 = float(entry_motor2.get())
	    	tar_joint3 = float(entry_motor3.get())
	    	tar_joint4 = float(entry_motor4.get())
	    	cur_joint2, cur_joint3, cur_joint4 = read_present_position()
	    	
	    	target_pulse = int(degree_to_pulse(tar_joint1)
...
def put_val():
	...
	global gain3
	global gain4
	gain2 = float(entry_motor2.get())
	gain3 = float(entry_motor3.get())
	gain4 = float(entry_motor4.get())
	"""
	tar_joint2 = float(entry_motor2.get())
	tar_joint3 = float(entry_motor3.get())
	tar_joint4 = float(entry_motor4.get())
...
def read_kp():
	for id in [ID2, ID3, ID4] :
		kp = send_can0(id, READ_2_BYTE, INDEX_POSITION_KP, 0)
		print(f"current joint{id - 0x600} kp : {kp}")
	

def set_kp(kp2, kp3, kp4):
	print(f"set_kp joint 2,3,4 : ({kp2}, {kp3}, {kp4})")
	send_can0(ID2, SET_2_BYTE, INDEX_POSITION_KP, kp2)
	send_can0(ID3, SET_2_BYTE, INDEX_POSITION_KP, kp3)
	send_can0(ID4, SET_2_BYTE, INDEX_POSITION_KP, kp4)
	time.sleep(1)
	read_kp()
...
def read_error():
    ...
    for servo_id in [ID2, ID3, ID4]:
        error_code = send_can0(servo_id, READ_2_BYTE, INDEX_ERROR_CODE, 0)
...
def fault_reset():
	...
	for servo_id in [ID2, ID3, ID4]:
		ret = send_can0(servo_id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x80)
		ret = send_can0(servo_id, SET_2_BYTE, INDEX_CONTROL_WORD, 0x00)
...
tk.Label(root, text="Enter position for Motor 4 (degrees):").grid(row=12, column=0, padx=5, pady=5)
entry_motor4 = tk.Entry(root)
entry_motor4.insert(0, "0")
entry_motor4.grid(row=12, column=1, padx=5, pady=5)
...
"""
# Input kp
tk.Label(root, text="Enter kp for Motor 3:").grid(row=16, column=0, padx=5, pady=5)
entry_kp2 = tk.Entry(root)
entry_kp2.insert(0, "6000")
entry_kp2.grid(row=16, column=1, padx=5, pady=5)

tk.Label(root, text="Enter kp for Motor 4:").grid(row=17, column=0, padx=5, pady=5)
