...
def servo_set_digital_output_state(pin, state):
    cur_do = servo_get_digital_output_state()
    ...
    set_sdo(ID1, SET_2_BYTE, OD_SERVO_CONTROL_WORD, 0x00, new_do)
...
def servo_set_interpolation_data(position, time, velocity):
    ...
    print_yellow(f"servo pvt wr: {tar_pulse_1},{scaled_velocity},{time}")
    print(f"servo pvt wr: {servo_pulses_to_degrees(tar_pulse_1)}, {servo_pulses_to_degrees(scaled_velocity/10)},{time}")
...
def servo_pvt_angle(tar_angle_1, t_ms, dt=100):
    ...
    print(f"cur_angle_1: {cur_angle_1}, tar_angle_1: {tar_angle_1}")
    print(f"servo_pvt_angle: cur_z={cur_z}, tar_z={tar_z}, t_ms={t_ms}, dt={dt}")
