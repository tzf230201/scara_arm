...
selected_pvt_path = tk.StringVar(value="")  # <== menyimpan path PVT yang dipilih
...
def choose_pvt():
    ...
    if file_path:
        ...
        print("PVT selected:", file_path)

def request_mode():
    send("request_mode", motor=current_motor())

# ==== UPDATE: Dancing now sends CSV path ====
def send_dancing():
    pvt_path = selected_pvt_path.get()
    send("dancing", motor=current_motor(), pvt_path=pvt_path)
...
tk.Button(root, text="Dancing", bg="green", fg="white",
          command=send_dancing).grid(row=13, column=1, sticky="ew")
...
tk.Button(root, text="OUT2 On",
          command=lambda: send("out2_active", motor=current_motor())).grid(row=16, column=0, sticky="ew")
tk.Button(root, text="OUT2 Off",
          command=lambda: send("out2_nonactive", motor=current_motor())).grid(row=16, column=1, sticky="ew")

# ==== NEW BUTTONS ====
tk.Button(root, text="Choose PVT", bg="lightblue",
          command=choose_pvt).grid(row=17, column=0, sticky="ew")
tk.Button(root, text="Request Mode", bg="lightgreen",
          command=request_mode).grid(row=17, column=1, sticky="ew")
